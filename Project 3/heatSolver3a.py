...
def solve(T0, uNorm, uH, uWF, delta_x, relax_w, iterations):
    ...
    if rank == 0:
        ...
        while (its < iterations):
            ...
            data_3_new = np.empty(int((x_len/2))**2, dtype = 'd')
            comm.Recv(data_3_new, source=3, tag = 42)

            its = its + 1
            
            data_1_all = relax_w*data_1_new + (1-relax_w)*data_1_all
            data_2_all = relax_w*data_2_new + (1-relax_w)*data_2_all
            data_3_all = relax_w*data_3_new + (1-relax_w)*data_3_all
            data_1 = data_1_all[(edges(x_len,x_len)[1])]
            data_2 = data_2_all[edges(x_len,x_len)[3]]
            data_3 = data_3_all[edges(int(x_len/2), int(x_len/2))[3]]

        plot3a(data_1_all, u_new, data_2_all, data_3_all, x_len, y_len)
